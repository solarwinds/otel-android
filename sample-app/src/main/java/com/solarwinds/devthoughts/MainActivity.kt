...
class MainActivity : AppCompatActivity() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    binding.appBarMain.fab.setOnClickListener { view -> showInputDialog(view) }
    ...
    appBarConfiguration =
      AppBarConfiguration(setOf(R.id.nav_home, R.id.nav_gallery, R.id.nav_slideshow), drawerLayout)
    ...
    lifecycleScope.launch {
      dataStore.data
        .map { settings -> settings[sessionIdPreferenceKey] ?: "unset" }
        .collectLatest { sessionId = it }
    }

    thoughtCounter =
      this.solarwindsRum().meter(this.meterProviderName).counterBuilder("thought.count").build()
  }
  ...
  private fun showInputDialog(view: View) {
    ...
    AlertDialog.Builder(this)
      ...
      .setPositiveButton("Submit") { dialog, _ ->
        ...
        dialog.cancel()
        Snackbar.make(view, msg, Snackbar.LENGTH_LONG).setAnchorView(R.id.fab).show()
      }
